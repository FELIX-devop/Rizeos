 ...
 * SeekerInboxPage
 * 
 * Page for job seekers to view messages including announcements.
 */
export default function SeekerInboxPage() {
  ...
  const loadMessages = async () => {
    setLoading(true);
    try {
      const data = await getSeekerInbox(token);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/70">Messages and announcements</p>
        </div>
        ...
        {!loading && messages.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-white/50">
              You'll see announcements and messages from recruiters here
            </p>
          </div>
        )}
