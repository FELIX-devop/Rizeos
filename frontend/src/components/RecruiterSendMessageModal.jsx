 ...
 * 
 * Modal component for job seekers to send messages to recruiters with job context.
 */
export default function RecruiterSendMessageModal({ isOpen, onClose, recruiter, job }) {
  const { token } = useAuth();
  ...
  const handleSend = async (e) => {
    ...
    if (!recruiter || !recruiter.id) {
      toast.error('Recruiter information is missing');
      return;
    }

    const jobId = job?.id || job?._id;
    if (!job || !jobId) {
      toast.error('Job context is required. Please select a job first.');
      return;
    }

    setSending(true);
    try {
      await sendMessage(token, message.trim(), recruiter.id, 'recruiter', jobId);
          ...
          >
            <div className="glass rounded-2xl p-6 w-full max-w-md space-y-4">
              ...
              <form onSubmit={handleSend} className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="block text-sm text-white/70">Job</label>
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10 text-white">
                    {job ? (job.title || 'N/A') : 'No job selected'}
                  </div>
                  {!job && (
                    <p className="text-xs text-yellow-400">Please select a job from the recruiter's profile first.</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder="Enter your message about this job..."
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    disabled={sending || !message.trim() || !job || (!job.id && !job._id)}
