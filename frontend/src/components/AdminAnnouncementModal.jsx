...
import { createAnnouncement } from '../services/api.js';
import { toast } from 'sonner';

/**
 * AdminAnnouncementModal
 * 
 * Modal component for admin to send global announcements.
 */
export default function AdminAnnouncementModal({ isOpen, onClose }) {
  const { token } = useAuth();
  ...
  const handleSend = async (e) => {
    ...
    setSending(true);
    try {
      await createAnnouncement(token, message);
              ...
              <form onSubmit={handleSend} className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-white/50 mt-1">
                    This announcement will be sent to all recruiters and job seekers.
                  </p>
                </div>
